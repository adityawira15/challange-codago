...
router.get('/', function(req, res, next) {
  ...
  if(req.url == '/'){
    db.all('SELECT * FROM bread LIMIT 3 OFFSET 0', (err, row) => {
      db.all('SELECT * FROM bread', (err, leng) => {
        res.render('index', {
          data: row,
          leng: leng.length,
          url: req.url,
    ...
    db.all(`SELECT * FROM bread LIMIT 3 OFFSET ${req.query.page*3}`, (err, row) => {
      db.all('SELECT * FROM bread', (err, leng) => {
        res.render('index', {
          data: row,
          leng: leng.length,
...
router.get('/edit/:id', function(req, res){
  db.all(`SELECT * FROM bread WHERE id = ${req.params.id}`, (err, row)=>{
